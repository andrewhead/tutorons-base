...
the explanations into arbitrary web pages.
...
Now we'll mount your Tutoron as a module into the server
as the `tutorons/modules/java_classes` directory:
...
Let's say we want to make pieces of documentaion about classes
from the Java API available wherever those classes appear on
a web page.  Let's customize the Tutoron so that it can
find classes from the Java API and provide this documentation.
...
an insightful sentence about that API.  Make a new file names
`insights.py` in the `tutorons/modules/java_classes/` directory. 
Paste this list into that file.
...
In the next two steps, we'll *detect* all code that refers
to the Java API classes for which we have insights, and
then create *explanations* that a web page visitor can see
in a tooltip whenever they see that Java API class mentioned.
...
We can see the new test fail when you run it:
...
the code in the `explain_code` method with this:
